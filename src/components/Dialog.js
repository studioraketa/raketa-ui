...
export const ModalWrapper = styled.div`
  ...
  box-shadow: 0 0 ${em(3)} rgba(0, 0, 0, 0.4);
  z-index: 19;
...
export default ({
  ...
  dialogSize
    ...
    <Modal open={open} onClose={onClose}>
      ...
      <ModalWrapper
        width={width}
        className={`${dialogSize === 'large' ? 'dialog-large' : ''}`}
      >
        <ModalHeader title={title} showClose onClose={onClose} />
        <ModalContent
          style={dialogSize === 'large' ? {} : { maxHeight: `${em(30)}` }}
        >
          {children}
        </ModalContent>
        {(primaryLabel || secondaryLabel) && (
          <ModalFooter>
            {primaryLabel && (
              <Button type='button' primary onClick={onPrimary}>
                {primaryLabel}
              </Button>
            )}
            {secondaryLabel && (
